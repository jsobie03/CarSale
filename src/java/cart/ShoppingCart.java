...
public class ShoppingCart {
     ...
     */
    public synchronized void addItem(Car car) {

        boolean newItem = true;

        for (ShoppingCartItem scItem : items) {

            if (!(null == scItem.getCar().getId()?car.getId() == null : scItem.getCar().getId().equals(car.getId()))) {
            } else {
                newItem = false;
        ...
        if (newItem) {
            ShoppingCartItem scItem = new ShoppingCartItem(car);
            items.add(scItem);
        }
    }
     ...
     *
     * @param car the <code>Car</code> that defines the type of shopping cart item
     * @param quantity the number which the <code>ShoppingCartItem</code> is updated to
     * @see ShoppingCartItem
     */
    public synchronized void update(Car car, String quantity) {
        ...
        if (qty >= 0) {
            ...
            for (ShoppingCartItem scItem : items) {
                ...
                if (Objects.equals(scItem.getCar().getId(), car.getId())) {
    ...
    public synchronized double getSubtotal() {
        ...
        for (ShoppingCartItem scItem : items) {
            ...
            Car car = (Car) scItem.getCar();
            amount += (scItem.getQuantity() * car.getPrice().doubleValue());
        }
